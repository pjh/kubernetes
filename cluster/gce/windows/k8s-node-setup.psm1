...
function DownloadAndInstall-TpmAuthPlugin {
  if (-not (Test-NodeUsesTpmBootstrap ${kube_env})) {
    Log-Output 'Skipping download of TPM auth plugin'
    return
  }
  ...
  if (-not ($kube_env.ContainsKey('EXEC_AUTH_PLUGIN_LICENSE_URL') -and
    ...
    Log-Output -Fatal ("Missing one or more kube-env keys needed for " +
                       "downloading TPM auth plugin: $(Out-String $kube_env)")
  }
  # TODO(pjh): for testing, set:
  #   URL: https://storage.googleapis.com/peterhornyack-gke-dev-bucket/gke-exec-auth-plugin.exe
  #   SHA1: 1BFA741134DFE91F8DC7B79C56EDDEDBFE022EC9
  #   LICENSE_URL: https://storage.googleapis.com/gke-prod-binaries/gke-exec-auth-plugin/054a22fb9c799a7a6cf064e71ecfe9db6a611ad2/LICENSE
  MustDownload-File `
...
# Required ${kube_env} keys:
#   CA_CERT
# ${kube_env} keys that can be omitted for nodes that use TPM-based
# authentication:
#   KUBELET_CERT
#   KUBELET_KEY
function Create-NodePki {
  ...
  # On TPM-based nodes KUBELET_CERT and KUBELET_KEY will not be present -
  # TPM_BOOTSTRAP_CERT and TPM_BOOTSTRAP_KEY will be set instead.
  if (Test-NodeUsesTpmBootstrap ${kube_env}) {
    Log-Output 'Skipping KUBELET_CERT and KUBELET_KEY for TPM-based node'
    return
  }
...
# Creates a kubelet kubeconfig at $env:KUBECONFIG that used the
# gke-exec-auth-plugin for TPM-based authentication.
#
# Create-NodePki() must be called first.
#
# Required ${kube_env} keys:
#   KUBERNETES_MASTER_NAME: the apiserver IP address.
function Write_TpmAuthPluginKubeconfig {
...
- name: exec-plugin-auth
  user:
    ...
    replace('APISERVER_ADDRESS', ${apiserverAddress}).`
    replace('CA_FILE_PATH', ${env:CA_FILE_PATH}).`
    replace('AUTH_PLUGIN', "${env:NODE_DIR}\gke-exec-auth-plugin.exe").`
    replace('PKI_DIR', ${env:PKI_DIR})
  Log-Output ("kubelet auth-plugin kubeconfig (non-bootstrap):`n" +
              "$(Get-Content -Raw ${env:KUBECONFIG})")
}

# Creates the kubelet kubeconfig at $env:BOOTSTRAP_KUBECONFIG for non-TPM
# nodes or at $env:KUBECONFIG for TPM-based nodes.
#
# Create-NodePki() must be called first.
#
# Required ${kube_env} keys:
...
function Start-WorkerServices {
  ...
  if (-not (Test-NodeUsesTpmBootstrap ${kube_env})) {
    Log-Output 'Using bootstrap kubeconfig for non-TPM Windows node'
    $kubelet_args = (${kubelet_args} +
                     "--bootstrap-kubeconfig=${env:BOOTSTRAP_KUBECONFIG}")
  }
  # TODO(pjh): run an E2E cluster (no TPMs!) and check that this output looks
  # good! Also run smoke-test.sh.
