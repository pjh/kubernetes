...
function Set-EnvironmentVars {
  ...
  $env_vars = @{
    ...
    "WINDOWS_CA_FILE" = ${kube_env}['WINDOWS_CA_FILE']
...
#   TODO(pjh): use EXEC_AUTH_PLUGIN_URL, EXEC_AUTH_PLUGIN_LICENSE_URL and
#   EXEC_AUTH_PLUGIN_SHA1 from kube-env.
function DownloadAndInstall-AuthPlugin {
  ...
  # TODO(pjh): use EXEC_AUTH_PLUGIN_URL, EXEC_AUTH_PLUGIN_LICENSE_URL and
  # EXEC_AUTH_PLUGIN_SHA1 from kube-env.
  MustDownload-File -OutFile "${env:NODE_DIR}\gke-exec-auth-plugin.exe" -URLs `
...
# Required ${kube_env} keys:
#   CA_CERT
# ${kube_env} keys that can be omitted for shielded nodes:
#   KUBELET_CERT
#   KUBELET_KEY
function Create-NodePki {
  ...
  if ($kube_env.ContainsKey('CA_CERT')) {
    ...
    Write_PkiData "${CA_CERT_BUNDLE}" ${env:WINDOWS_CA_FILE}
  }
  else {
...
- name: local
  cluster:
    ...
    certificate-authority: WINDOWS_CA_FILE
contexts:
- context:
    cluster: local
    user: kubelet
...
current-context: service-account-context'.`
    ...
    replace('WINDOWS_CA_FILE', ${env:WINDOWS_CA_FILE})
...
# Required ${kube_env} keys:
#   KUBERNETES_MASTER_NAME: the apiserver IP address.
function Write_AuthPluginKubeconfig {
...
function Start-WorkerServices {
  ...
  if (-not (Test-IsShieldedNode ${kube_env})) {
    Log-Output 'Using bootstrap kubeconfig for non-shielded Windows node'
    $kubelet_args = (${kubelet_args} +
                     "--bootstrap-kubeconfig=${env:BOOTSTRAP_KUBECONFIG}")
  }
  # TODO(peterhornyack): check that this output looks good!
